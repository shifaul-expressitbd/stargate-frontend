import { useCalendarPosition } from '@/components/utils/hooks/useCalendarPosition'
...
const Calendar: React.FC<CalendarProps> = ({
  ...
  const [currentDate, setCurrentDate] = useState(
    selectedDates && selectedDates.length > 0 ? selectedDates[0] : new Date()
  )
  ...
  const handleDateClick = (date: Date) => {
    ...
    if (mode === 'single') {
      // Set to start of day for single selection
      ...
      // For multi selection, use start of day
      const startOfDay = new Date(date)
      startOfDay.setHours(0, 0, 0, 0)
      newDates = isSelected
      ...
      if (!tempStartDate) {
        ...
        const start = tempStartDate < date ? tempStartDate : new Date(date)
        const end = tempStartDate < date ? new Date(date) : tempStartDate
        ...
        newDates = [normalizedStart, normalizedEnd]
        closeCalendar()
      }
    }
