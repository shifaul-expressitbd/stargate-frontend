  ...
  const renderDaysForMonth = (monthOffset = 0) => {
    ...
    for (let i = 1; i <= endDay; i++) {
      ...
      const dayClass = twMerge(
        'min-h-4 h-8 min-w-4 w-8 aspect-square flex items-center justify-center rounded text-xs md:text-sm transition-colors dark:text-white',
        isToday && 'font-bold border border-primary dark:border-secondary ',
        isPastDate && 'line-through text-gray-400 dark:text-gray-300',
        isSelected && !isRangeStart && !isRangeEnd
          ? error
            ? 'bg-red-600 text-white'
            : 'bg-primary text-gray-900 hover:text-white'
          : isInRange
            ? error
              ? 'bg-red-100 text-red-800 dark:bg-red-900/50 dark:text-red-200'
              : 'bg-orange-100 text-orange-800 dark:bg-orange-900/50 dark:text-orange-200'
            : 'hover:bg-gray-100 dark:hover:bg-gray-700',
        isRangeStart && 'bg-primary text-gray-900 hover:text-white',
        isRangeEnd && 'bg-primary text-gray-900 hover:text-white',
